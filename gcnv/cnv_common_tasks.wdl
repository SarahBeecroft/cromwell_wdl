task CollectCounts {
    ...
    String counts_filename = "${base_filename}.counts.hdf5"

    command <<<
        set -e
		module load GATK/4.1.0.0-Java-1.8.0_121

        gatk --java-options "-Xmx4000m" CollectReadCounts \
            -L ${intervals} \
            --input ${bam} \
            --reference ${ref_fasta} \
            --format HDF5 \
...
task ScatterIntervals {
    ...
    command <<<
        ...
        
        {
            ...
            gatk --java-options "-Xmx4000m" IntervalListTools \
                --INPUT ${interval_list} \
...
task PostprocessGermlineCNVCalls {
    ...
    command <<<
        ...
        gatk --java-options "-Xmx7000m" PostprocessGermlineCNVCalls \
            $calls_args \
            $model_args \
