...
task CramToBamTask {
  ...
  Int? cpus
  Int? mem_mb
  Int machine_mem_mb = select_first([mem_mb, 32000])
  Int cpus_n = select_first([cpus, 8])
  ...
  runtime {
    cpus: cpus_n
    requested_memory: machine_mem_mb
 }
  output {
...
task HaplotypeCaller {
  ...
  Int? cpus
  Int? mem_mb
  Int machine_mem_mb = select_first([mem_mb, 16000])
  Int cpus_n = select_first([cpus, 4])
  ...
  set -e
    ...
    gatk --java-options "-Xmx${command_mem_mb}m ${java_opt}" \
      HaplotypeCaller \
      -R ${ref_fasta} \
      -I ${input_bam} \
      -L ${interval_list} \
