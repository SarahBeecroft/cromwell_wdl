...
# WORKFLOW DEFINITION 
workflow Bam_to_Ubam_to_Gvcf_GATK4 {
  ...
  call SamToFastqAndBwaMem {
    input:
      input_bam = BamToUbam.unmapped_bam,
  ...
  call MergeBamAlignment {
    input:
      unmapped_bam = BamToUbam.unmapped_bam,
      bwa_commandline = bwa_commandline,
  ...
  output {
    File unmapped_bam = BamToUbam.unmapped_bam
    File revert_bam_metrics = BamToUbam.revert_bam_metrics
...
task BamToUbam {
  ...
  command {
    ...
    java -Xmx8G -jar $EBROOTPICARD/picard.jar RevertSam \
    I=${input_bam} \
    O=${outpref}.unmarked_unmapped.bam \
    SANITIZE=true \
    ...
    I=${outpref}.unmarked_unmapped.bam \
    O=${outpref}.unmapped.bam \
    M=${outpref}_revertmark.metrics.txt
  }
  ...
  output {
    ...
    File revert_bam_metrics = "${outpref}_revertmark.metrics.txt"
  }
}
...
task MarkDuplicates {
 ...
 # While query-grouped isn't actually query-sorted, it's good enough for MarkDuplicates with ASSUME_SORT_ORDER="queryname"
  command {
    gatk --java-options "-Dsamjdk.compression_level=${compression_level} -Xms40000m" \
      MarkDuplicates \
      --INPUT ${input_bam} \
      --OUTPUT ${output_bam_basename}.bam \
...
task GatherBamFiles {
  ...
  command {
    gatk --java-options "-Dsamjdk.compression_level=${compression_level} -Xms2000m" \
      GatherBamFiles \
